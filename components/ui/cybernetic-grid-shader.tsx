...
const CyberneticGridShader = ({ isDark }: { isDark: boolean }) => {
  ...
  useEffect(() => {
    ...
    // pega os hex das tuas variáveis CSS
    ...
    const fragmentShader = `
      ...
      // cores da paleta injectadas como uniforms
      ...
      void main() {
        ...
        // usa uColorGrid em vez de hardcoded
        vec3 color = uColorGrid * line * (0.5 + sin(t * 2.0) * 0.2);

        float energy = sin(uv.x * 20.0 + t * 5.0)
                     * sin(uv.y * 20.0 + t * 3.0);
        energy = smoothstep(0.8, 1.0, energy);

        // usa uColorEnergy em vez de hardcoded
        color += uColorEnergy * energy * line;

        float glow = smoothstep(0.1, 0.0, mouseDist);

        // usa uColorGlow em vez de hardcoded
        color += uColorGlow * glow * 0.5;
    ...
    const uniforms = {
      ...
      // cores da paleta
      uColorGrid:   { value: hexToVec3(colorAccent || '#3A86FF') },
      uColorEnergy: { value: hexToVec3(colorButton || '#0529BE') },
      uColorGlow:   { value: new THREE.Vector3(1, 1, 1) },
    };
